...
PayCard::PayCard(std::string text): Card(text){
    message = text;
}

void PayCard::effect(std::string player_name, int amount, std::string optional_receiver){
...
GetMoneyCard::GetMoneyCard(std::string text) : Card(text){
    message = text;
}

void GetMoneyCard::effect(std::string player_name, int amount, std::string optional_receiver){
    std::cout << message << std::endl;
    std::cout << "Player " << player_name << " receives " << amount << "$" << std::endl;
}

GetOutOfJailCard::GetOutOfJailCard(std::string text) : Card(text){
    message = text;
}

void GetOutOfJailCard::effect(std::string player_name, int amount, std::string optional_receiver){
    std::cout << message << std::endl;
    std::cout << "Player " << player_name << " receives GetOutOfJailCard" << std::endl;
}

MoveCard::MoveCard(std::string text) : Card(text){
    message = text;
}

void MoveCard::effect(std::string player_name, int amount, std::string optional_receiver){
    std::cout << message << std::endl;
    if (amount != 11){
    ...
    if (input_file){
        while(std::getline(input_file, line)){
            ...
            if(type == "7PayCard"){
                ...
                card_collection.push_back(std::make_shared<PayCard>(text));
            }
            if (type == "12GetMoneyCard") {
                card_collection.push_back(std::make_shared<GetMoneyCard>(text));
            }
...
void Deck<CardType>::save_deck(std::string file){
    ...
    for (const auto& card : deck) {
        oss << typeid(*card).name() << "," << card->message << ";" << std::endl;
    }
    text = oss.str();
